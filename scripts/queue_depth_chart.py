...
if __name__ == "__main__":
    candidates = [Path.cwd(), Path("/content/DoS-Stress-Testing"), Path("/content/DoS-Stress-Testing-main")]
    repo = next((p for p in candidates if (p / "gateway/app.py").is_file()), Path.cwd())
    def locate(run):
        options = [repo / "runs" / run / "gateway_samples.jsonl", Path("/content/runs") / run / "gateway_samples.jsonl"]
        return next((p for p in options if p.is_file()), options[0])
    plot_queue_depth({"OFF": locate("flood-off-8"), "ON": locate("flood-on-fresh")}, repo / "results")
